...
const IndividualDashboard: React.FC = () => {
  ...
  if (isCreating) {
    return (
       <div className="max-w-xl mx-auto bg-white dark:bg-slate-900 p-6 md:p-8 rounded-3xl shadow-xl border border-slate-200 dark:border-slate-800 animate-fade-in my-8">
        <div className="flex justify-between items-center mb-6">
            <h2 className="text-2xl font-bold text-slate-900 dark:text-white flex items-center gap-2">
                <PiggyBank className="text-purple-500" /> {t('ind.create_btn')}
            </h2>
            <button onClick={() => setIsCreating(false)} className="p-2 bg-slate-100 dark:bg-slate-800 rounded-full hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-500 dark:text-slate-400">
                <X size={20} />
            </button>
        </div>
        ...
        <div className="space-y-6">
            {/* 1. Name */}
            <div>
                <label className="block text-sm font-bold text-slate-700 dark:text-slate-300 mb-2">{t('ind.form_name')}</label>
                <input 
                    type="text" 
                    value={name} 
                    onChange={(e) => setName(e.target.value)}
                    placeholder="Ex: Novo Telefone"
                    className="w-full p-4 border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 dark:text-white rounded-xl focus:ring-2 focus:ring-purple-500 outline-none transition-all font-medium text-base"
                />
            </div>
            ...
            <div className="bg-slate-50 dark:bg-slate-800/50 p-4 rounded-xl border border-slate-100 dark:border-slate-800 grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    <label className="block text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wide mb-2">{t('ind.form_freq')}</label>
                ...
                <div>
                    <label className="block text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wide mb-2">{t('ind.form_contribution')}</label>
                    <div className="relative">
                        ...
                        <input 
                            ...
                            placeholder="0"
                            className="w-full p-3 pl-10 border border-slate-200 dark:border-slate-700 dark:bg-slate-800 rounded-lg focus:ring-2 focus:ring-purple-500 outline-none font-bold text-slate-900 dark:text-white text-lg"
                        />
                    </div>
                </div>
            </div>
            ...
            <div>
                <label className="block text-sm font-bold text-slate-700 dark:text-slate-300 mb-2">Planning Method</label>
                <div className="grid grid-cols-3 gap-2 mb-4">
                     <button 
                        ...
                        className={`py-3 px-2 rounded-xl text-xs font-bold border flex flex-col items-center gap-1 transition-all ${
                            goalMode === 'manual' ? 'bg-slate-800 dark:bg-slate-700 text-white border-slate-800 dark:border-slate-700' : 'bg-white dark:bg-slate-900 text-slate-500 dark:text-slate-400 border-slate-200 dark:border-slate-700'
                        }`}
                     >
                     ...
                     <button 
                        ...
                        className={`py-3 px-2 rounded-xl text-xs font-bold border flex flex-col items-center gap-1 transition-all ${
                            goalMode === 'date' ? 'bg-slate-800 dark:bg-slate-700 text-white border-slate-800 dark:border-slate-700' : 'bg-white dark:bg-slate-900 text-slate-500 dark:text-slate-400 border-slate-200 dark:border-slate-700'
                        }`}
                     >
                     ...
                     <button 
                        ...
                        className={`py-3 px-2 rounded-xl text-xs font-bold border flex flex-col items-center gap-1 transition-all ${
                            goalMode === 'count' ? 'bg-slate-800 dark:bg-slate-700 text-white border-slate-800 dark:border-slate-700' : 'bg-white dark:bg-slate-900 text-slate-500 dark:text-slate-400 border-slate-200 dark:border-slate-700'
                        }`}
                     >
                ...
                <div className="p-4 bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-800 rounded-xl space-y-4">
                    ...
                    <div>
                        ...
                        <input 
                             ...
                             className="w-full p-3 border border-slate-300 dark:border-slate-700 rounded-lg text-sm font-medium bg-white dark:bg-slate-800 dark:text-white"
                        />
                    </div>

                    {goalMode === 'manual' && (
                        <div className="animate-fade-in">
                            <label className="block text-xs font-bold text-slate-500 dark:text-slate-400 mb-1">{t('ind.form_target')}</label>
                            <div className="relative">
                                <span className="absolute left-3 top-4 text-slate-400 text-xs font-bold">{t('common.currency')}</span>
                                <input 
                                    type="number" 
                                    ...
                                    placeholder="0"
                                    className="w-full p-3 pl-10 border border-slate-300 dark:border-slate-700 dark:bg-slate-800 rounded-lg text-lg font-bold text-purple-700 dark:text-purple-400"
                                />
                            </div>
                        </div>
                    )}
                    ...
                    {goalMode === 'date' && (
                        <div className="animate-fade-in">
                            ...
                            <input 
                                 ...
                                 className="w-full p-3 border border-slate-300 dark:border-slate-700 rounded-lg text-sm font-medium bg-white dark:bg-slate-800 dark:text-white"
                            />
                            <div className="mt-3 text-xs text-slate-500 dark:text-slate-400 flex justify-between items-center bg-white dark:bg-slate-900 p-2 rounded border border-slate-100 dark:border-slate-800">
                                <span>Estimated Saves:</span>
                                <span className="font-bold text-slate-800 dark:text-slate-200">{occurrences || 0} times</span>
                            </div>
                        </div>
                    )}
                    ...
                    {goalMode === 'count' && (
                        <div className="animate-fade-in">
                            <label className="block text-xs font-bold text-slate-500 dark:text-slate-400 mb-1">Number of times to save</label>
                            <input 
                                 type="number" 
                                 inputMode="numeric"
                                 value={occurrences} 
                                 ...
                                 placeholder="0"
                                 className="w-full p-3 border border-slate-300 dark:border-slate-700 dark:bg-slate-800 rounded-lg text-lg font-bold text-slate-800 dark:text-white"
                            />
                             <div className="mt-3 text-xs text-slate-500 dark:text-slate-400 flex justify-between items-center bg-white dark:bg-slate-900 p-2 rounded border border-slate-100 dark:border-slate-800">
                                <span>Est. End Date:</span>
                                <span className="font-bold text-slate-800 dark:text-slate-200">{new Date(endDate).toLocaleDateString()}</span>
                            </div>
                        </div>
                    )}
                </div>
            ...
            {/* Summary Footer */}
            <div className="bg-purple-50 dark:bg-purple-900/20 border border-purple-100 dark:border-purple-800/50 p-4 rounded-xl flex items-center justify-between">
                <div>
                     <div className="text-xs text-purple-600 dark:text-purple-400 font-bold uppercase mb-1">Total Accumulated</div>
                     <div className="text-2xl font-bold text-purple-800 dark:text-purple-300">{formatCurrency(Number(targetAmount) || 0)}</div>
                </div>
                <div className="bg-white dark:bg-slate-800 p-2 rounded-full text-purple-500 dark:text-purple-400 shadow-sm">
                    <Calculator size={20} />
                </div>
            </div>
            ...
            {/* Method */}
            <div>
                <label className="block text-sm font-bold text-slate-700 dark:text-slate-300 mb-2">{t('ind.form_method')}</label>
  ...
  return (
    <div className="space-y-8 animate-fade-in pb-12">
            ...
            <div className="fixed inset-0 bg-black/50 z-[9999] flex items-center justify-center p-4">
                <div className="bg-white dark:bg-slate-900 rounded-3xl p-6 max-w-sm w-full shadow-2xl animate-fade-in border dark:border-slate-800">
                    ...
                    <div className="bg-slate-50 dark:bg-slate-800/50 p-4 rounded-xl border border-slate-200 dark:border-slate-700 mb-4">
                        ...
                        <div className="text-xl font-bold text-slate-900 dark:text-white">{formatCurrency(calculateBalance(withdrawStick.transactions))}</div>
                    </div>
                    ...
                    <input 
                        ...
                        className="w-full p-3 border border-slate-300 dark:border-slate-700 dark:bg-slate-800 dark:text-white rounded-xl mb-2 font-bold text-lg"
                    />
                    ...
                    <div className="flex gap-2">
                        <button 
                            ...
                            className="flex-1 py-3 bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-600 dark:text-slate-300 font-bold rounded-xl"
                        >
                            {t('ind.withdraw_cancel')}
                        </button>
                        <button 
                            onClick={handleWithdraw}
                            className="flex-1 py-3 bg-slate-900 dark:bg-slate-700 hover:bg-slate-800 dark:hover:bg-slate-600 text-white font-bold rounded-xl"
                        >
        ...
        <div className="flex justify-between items-center">
            <div>
                <h1 className="text-3xl font-bold text-slate-900 dark:text-white tracking-tight">{t('ind.title')}</h1>
                <p className="text-slate-500 dark:text-slate-400 mt-1">{t('ind.subtitle')}</p>
            </div>
            <button 
                onClick={() => setIsCreating(true)}
                className="bg-purple-600 hover:bg-purple-700 text-white px-4 py-2 md:px-6 md:py-3 rounded-xl font-bold shadow-lg shadow-purple-200 dark:shadow-none transition-all flex items-center gap-2"
            >
        ...
        {sticks.length === 0 ? (
            <div className="text-center py-20 bg-white dark:bg-slate-900 rounded-3xl border border-dashed border-slate-300 dark:border-slate-700 shadow-sm">
                <div className="w-20 h-20 bg-purple-50 dark:bg-purple-900/20 rounded-full flex items-center justify-center mx-auto mb-6 text-purple-400">
                    <PiggyBank size={40} />
                </div>
                <h3 className="text-xl font-bold text-slate-800 dark:text-slate-200 mb-2">{t('ind.no_active')}</h3>
                <p className="text-slate-500 dark:text-slate-400 mb-8 max-w-sm mx-auto">{t('ind.start_desc')}</p>
            </div>
        ) : (
                ...
                {sticks.map(stick => {
                    ...
                    return (
                        <div 
                            ...
                            className={`bg-white dark:bg-slate-900 rounded-2xl p-6 border transition-all cursor-pointer ${isExpanded ? 'border-purple-300 dark:border-purple-500 shadow-xl ring-2 ring-purple-50 dark:ring-purple-900/20 col-span-1 md:col-span-2' : 'border-slate-200 dark:border-slate-800 shadow-sm hover:shadow-lg'}`}
                        >
                            {/* Header */}
                            <div className="flex justify-between items-start mb-4">
                                <div>
                                    <div className="flex items-center gap-2 mb-1">
                                        <h3 className="text-lg font-bold text-slate-900 dark:text-white">{stick.name}</h3>
                                ...
                                <button onClick={(e) => requestDelete(stick.id, e)} className="text-slate-300 dark:text-slate-600 hover:text-red-500 dark:hover:text-red-400 transition-colors p-2">
                                    <Trash size={18} />
                                </button>
                            </div>
                            ...
                            <div className="mb-4">
                                <div className="flex justify-between items-end mb-2">
                                    <div className="text-3xl font-bold text-slate-900 dark:text-white">{formatCurrency(balance)}</div>
                                    <div className="text-xs font-bold text-slate-500 dark:text-slate-400">{Math.round(progress)}% of {formatCurrency(target)}</div>
                                </div>
                            ...
                            <div className="grid grid-cols-2 gap-3 mb-4">
                                <button 
                                    ...
                                    className="bg-emerald-50 dark:bg-emerald-900/20 text-emerald-700 dark:text-emerald-400 hover:bg-emerald-100 dark:hover:bg-emerald-900/40 py-3 rounded-xl font-bold text-sm transition-colors flex items-center justify-center gap-2 border border-emerald-100 dark:border-emerald-800/50"
                                >
                                    <Plus size={16} /> {t('ind.deposit')}
                                </button>
                                <button 
                                    onClick={(e) => { e.stopPropagation(); setWithdrawStick(stick); }}
                                    className="bg-slate-50 dark:bg-slate-800 text-slate-700 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-700 py-3 rounded-xl font-bold text-sm transition-colors border border-slate-100 dark:border-slate-700"
                                >
                                    {t('ind.withdraw')}
                                </button>
                            </div>
                            ...
                            {isExpanded && (
                                <div className="border-t border-slate-100 dark:border-slate-800 pt-4 mt-4 animate-fade-in">
                                     <div className="grid md:grid-cols-2 gap-6">
                                         <div>
                                            <h4 className="text-xs font-bold text-slate-400 dark:text-slate-500 uppercase mb-3">{t('ind.timeline')}</h4>
                                            <ul className="space-y-4 relative pl-4 border-l-2 border-slate-100 dark:border-slate-800">
                                                <li className="relative">
                                                    <div className="absolute -left-[21px] top-1 w-3 h-3 rounded-full bg-purple-500 ring-4 ring-white dark:ring-slate-900"></div>
                                                    <div className="text-sm font-bold text-slate-800 dark:text-slate-200">{t('ind.timeline_start')}</div>
                                                    <div className="text-xs text-slate-500 dark:text-slate-400">{new Date(stick.createdAt).toLocaleDateString()}</div>
                                                </li>
                                                <li className="relative">
                                                     <div className="absolute -left-[21px] top-1 w-3 h-3 rounded-full bg-slate-300 dark:bg-slate-600 ring-4 ring-white dark:ring-slate-900"></div>
                                                     <div className="text-sm font-bold text-slate-800 dark:text-slate-200">{t('ind.remaining')}</div>
                                                     <div className="text-xs text-slate-500 dark:text-slate-400">{formatCurrency(remaining)} ({estimatedPeriods} {stick.frequency === 'DAILY' ? t('ind.days') : stick.frequency === 'WEEKLY' ? t('ind.weeks') : t('ind.months')})</div>
                                                </li>
                                                <li className="relative">
                                                     <div className="absolute -left-[21px] top-1 w-3 h-3 rounded-full bg-emerald-400 ring-4 ring-white dark:ring-slate-900"></div>
                                                     <div className="text-sm font-bold text-emerald-700 dark:text-emerald-400">{t('ind.timeline_projected')}</div>
                                                     <div className="text-xs text-emerald-600 dark:text-emerald-500 font-bold">
                                         ...
                                         <div>
                                            <h4 className="text-xs font-bold text-slate-400 dark:text-slate-500 uppercase mb-3">{t('ind.history')}</h4>
                                            <div className="max-h-40 overflow-y-auto space-y-2 pr-2 custom-scrollbar">
                                                {stick.transactions && stick.transactions.length > 0 ? (
                                                    stick.transactions.map((tx) => (
                                                        <div key={tx.id} className="flex justify-between items-center text-sm p-2 bg-slate-50 dark:bg-slate-800/50 rounded-lg">
                                                            <div className="flex items-center gap-2">
                                                                <span className={`w-2 h-2 rounded-full ${tx.type === TransactionType.DEPOSIT ? 'bg-emerald-500' : 'bg-red-500'}`}></span>
                                                                <span className="text-slate-600 dark:text-slate-300">{tx.description}</span>
                                                            </div>
                                                            <div className={`font-bold ${tx.type === TransactionType.DEPOSIT ? 'text-emerald-600 dark:text-emerald-400' : 'text-slate-900 dark:text-white'}`}>
                                                                {tx.type === TransactionType.WITHDRAWAL ? '-' : '+'}{formatCurrency(tx.amount)}
                                                            </div>
                                                        </div>
                                                    ))
                                                ) : (
                                                    <div className="text-slate-400 dark:text-slate-500 text-xs text-center py-4 italic">No transactions yet</div>
                                                )}
                                            </div>
                                         </div>
                                     </div>
